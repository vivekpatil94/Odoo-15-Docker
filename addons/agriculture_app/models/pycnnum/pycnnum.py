...
LARGER_CHINESE_NUMBERING_UNITS_SIMPLIFIED: str = "億兆京垓秭穰溝澗正載"
"""億兆京垓秭穰溝澗正載
...
class NumberingSystem:
    ...
    def __init__(self, numbering_type: str = NUMBERING_TYPES[1]) -> None:
        ...
        digits = [ChineseNumberDigit(i, *v)
                  for i, v in enumerate(chinese_digits)]
...
def cn2num(
    ...
    def refine_symbols(
        ...
        if (
            ...
            integer_symbols = [system.digits[1]] + \
                integer_symbols  # type: ignore
    ...
    dec_str = "".join(
        [str(d.int_value)
         for d in dec_part if isinstance(d, ChineseNumberDigit)]
    )
